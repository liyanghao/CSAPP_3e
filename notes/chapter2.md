...
#### 第2.2.2小节 无符号数编码
让我们考虑一个有w位的整数数据。我们记位向量为$\vec{x}$来表示整个向量，或者为$[x_{w-1},x_{w-2},...,x_0]$来表示向量内的单独的位。

将向量$\vec{x}$看做是用二进制表示的数，则我们可以获得向量$\vec{x}$的无符号解释。

使用这种编码方法，每个位$x_i$的值是0或1，后一种情形($x_i$的值等于1)意味着$2^i$应包含为数值的一部分。
...
图2.12展示了函数$B2U_w$给出的映射。

在图2.12里，我们将位置i的每个位表示为一个向右的、长度为$2^i$的蓝色长条。然后，一个位向量对应的数值就是对应数位是1的长条的长度之和。

让我们来考虑w个bit位能表示的数值范围有多大。最小的数是0，位向量为$[00...0]$。最大的数是$2^w-1$，位向量为$[11...1]$。因此，定义函数$B2U_w$是映射$B2U_w:\{0,1\}^w\rightarrow \{UMax_w\}$，其中$UMax_w=\sum_0^{w-1}2^i=2^w-1$。
